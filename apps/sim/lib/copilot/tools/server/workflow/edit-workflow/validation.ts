...
/**
 * Common source-handle aliases produced by LLMs for linear edges.
 * These all map to the canonical `source` handle used in workflow state.
 */
const LINEAR_SOURCE_HANDLE_ALIASES = new Set([
  'source',
  'success',
  'default',
  'output',
...
function normalizeLinearSourceHandle(sourceHandle: string): string {
  const normalized = sourceHandle.trim().toLowerCase()
  return LINEAR_SOURCE_HANDLE_ALIASES.has(normalized) ? 'source' : sourceHandle
}
...
export function validateSourceHandleForBlock(
  ...
  const normalizedLinearHandle = normalizeLinearSourceHandle(sourceHandle)
  ...
  switch (sourceBlockType) {
    ...
    case 'router':
      if (normalizedLinearHandle === 'source') {
        return { valid: true, normalizedHandle: 'source' }
      }
...
export function validateTargetHandle(targetHandle: string): EdgeHandleValidationResult {
  ...
  if (normalized === 'target') {
    return { valid: true, normalizedHandle: 'target' }
  }

  // Common aliases generated by models for the canonical target handle.
